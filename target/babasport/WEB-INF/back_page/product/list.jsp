...
function isShow(pageNo,name,brandId,isShow){
    ...
    $("#tableForm").attr("action","isShow.do?pageNo=" + pageNo + "&name=" + name + "&brandId=" + brandId + "&isShow=" + isShow);
    $("#tableForm").submit();
}
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" class="pn-ltable">
	...
	<c:forEach items="${pagination.list}" var="entry">
		<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
			<td><input type="checkbox" name="ids" value="${entry.id}" onclick="Pn.checkHead()" /></td>
			...
			<td align="center">
			<a href="#" class="pn-opt">查看</a> | <a href="#" class="pn-opt">修改</a> | <a href="/product/delete.do?id=${entry.id }&name=${name}&isShow=${isShow}&brandId=${brandId}" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a> | <a href="/sku/list.do?productId=${entry.id}&pno=${entry.no}" class="pn-opt">库存</a>
			</td>
		</tr>
	</c:forEach>
	</tbody>
